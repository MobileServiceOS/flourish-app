...
platform is rebuilt — which is how `package.json` came to say `1.0.0` while
Xcode said `1.0`.

`npm run sync` stamps `MARKETING_VERSION` from `package.json` after `cap sync`,
next to the icons, which are restored for the same reason. To release a new
version, change `package.json` and sync.

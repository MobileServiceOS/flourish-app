...
| `package.json` | `flourish.ios.buildNumber` | 7 | bump per upload |
| Xcode → App → General | Version (`MARKETING_VERSION`) | 1.2.0 | stamped automatically |
| Xcode → App → General | Build (`CURRENT_PROJECT_VERSION`) | 7 | stamped automatically |

**1.0.2 is live on the App Store.** This build is an update, so the version has
to be higher than 1.0.2 and the build number higher than anything already
uploaded under it.
...
The **build number is stamped too**, which reverses an earlier decision. It has
to increase on every upload — a duplicate is rejected — and leaving it in Xcode
put it in the one directory that gets regenerated. It lives in `package.json`
under `flourish.ios.buildNumber`; bump it there and run `npm run sync`.

That is not theoretical, and it has now happened **twice**.

Before an earlier revision the project file read `MARKETING_VERSION = 1.0.1`
while `package.json` said `1.0.0` — a value typed into Xcode, in a directory git
does not track. Re-stamping put it back.

Preparing build 7 it happened again, in the dangerous direction. The project
read **1.2.0 / build 6** while `package.json` said 1.1.0 / build 5, and
`package.json` has never once said 1.2.0 in its entire history — so 1.2.0 was
typed into Xcode by hand as well. The stamp script refused to write the lower
number and said why, which is the whole reason it checks: stamping 1.1.0 over a
1.2.0 that may already have been uploaded would have looked like a clean build
and failed at upload, or worse, shipped a version lower than the one live.

**The resolution when the two disagree is to go UP, never down.** Because
`ios/` is untracked there is no history to tell whether 1.2.0 build 6 was ever
uploaded, and App Store Connect refuses a duplicate build number. So
`package.json` was moved to 1.2.0 / build 7 — higher than the live 1.0.2,
higher than anything the project file showed, and therefore safe whichever
turns out to be true. Skipping a version number costs nothing; colliding with
one costs a rejected upload.

If you ever type a version into Xcode, put it in `package.json` instead and run
`npm run sync`. Xcode's copy is scratch space.

---

## What's New (paste into App Store Connect)

*This is an **update**, not a first submission — 1.0.2 is live, so this box is
shown to customers and must not be left empty. Everything below is user-facing
and true of build 7; when a release ships nothing a customer would notice, say
...
worth, you pay the difference instead of being told you can't have it. Nothing
is ever turned down for being too expensive.

The ladder, all of it visible on the Rewards screen:

  70    Free drink
  100   $5 off any order
  120   Free side
  160   Free seafood mac
  250   Free lunch
  350   Free plate

Free lunch at 250 Petals is new, and a free plate now needs 350 instead of what
it used to.

Petals. Your rewards balance has its own name, so it is never mistaken for the
Perks you earn at the counter. They are two separate balances and they do not
combine. A Petal per dollar, one reward per order.

Your balance now lives with us, not just on your phone. Change phones, reinstall
the app, order from a tablet — the same balance follows your name and number. No
codes, no sign-up link, and we never text you.

Petals arrive even if you close the app. They used to be credited only while the
order screen was open, so paying at the counter with the app shut meant they
never showed up. Now the app squares up with the register every time you open it.

Tap any past order to see the whole thing. Every item with its sides and
flavours, exactly what you typed in the instructions box, the pickup window it
was given, the curbside car, what it cost, what it earned, and the order number
to read out at the counter.

Your balance and your order's status refresh the moment you look at them, rather
than whenever the app last felt like checking.

We're open 11AM to 10PM, every day. The app used to show a later closing time on
Fridays and Saturdays, which was wrong.

Menu: Curry Chicken is back on the list, and Seafood Fridays runs eight dishes.
```

...
| `package.json` | `flourish.ios.buildNumber` | 4 | bump per upload |
| Xcode → App → General | Version (`MARKETING_VERSION`) | 1.0.0 | stamped automatically |
| Xcode → App → General | Build (`CURRENT_PROJECT_VERSION`) | 4 | stamped automatically |
...
That is not theoretical. Before this revision the project file read
`MARKETING_VERSION = 1.0.1` while `package.json` said `1.0.0` — a value typed
into Xcode at some point, in a directory git does not track, disagreeing with
the authoritative one. The suite caught it, and re-stamping put it back to
1.0.0. Nothing has shipped yet, so 1.0.0 is still the right first version; if
1.0.1 was deliberate, change `package.json` and sync rather than editing
Xcode.
...
*First submission, so this box can be left empty — App Store Connect only shows
it on an update. Kept here because build 4 is a resubmission after the
screenshot rejection, and the text is needed the moment 1.0.1 ships.*
...
meant they never arrived. The app now checks your recent orders when it opens.

Curbside pickup, special instructions on any plate, and a menu that keeps up
with the kitchen: soups by the day, Seafood Fridays, and correct prices on the
platters.
```
